...
object ConnectedComponentBatch {
  ...
  def main(args: Array[String]) {
      ...
      try {
        ...
        def process {
          ...
          for (node1 <- nodes.seq.iterator) {
            ...
            inputVertexArray.+=((node1, Long.MaxValue))
          ...
          if (isFirst) {
            ...
            if (storage) {
              ...
              graph = Graph(users, relationships, Long.MaxValue, StorageLevel.MEMORY_ONLY_SER, StorageLevel.MEMORY_ONLY_SER)
            } else {
              graph = Graph(users, relationships, Long.MaxValue)
            }
            ...
            if (storage) {
              ...
              graph = Graph(newusers, relationships, Long.MaxValue, StorageLevel.MEMORY_ONLY_SER, StorageLevel.MEMORY_ONLY_SER)
            } else {
              graph = Graph(newusers, relationships, Long.MaxValue)
            }
          ...
          println("Done: " + total + " at : " + new Date() + " RF: " + rf + " lrsd " + relativeSD + "," + vertexProgramCounter.value + "," + sendMsgProgramCounter.value + "," + mergeMsgProgramCounter.value)
          if (total == batch) {
    ...
    def connectedComponent(graph: Graph[Long, Long]): Graph[Long, Long] = {
    ...
    def sendMessage(edge: EdgeTriplet[VertexId, Long]): Iterator[(VertexId, VertexId)] = {
